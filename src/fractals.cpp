...
void drawSierpinskiTriangle(GWindow& gw, double x, double y, double size, int order) {
    ...
    if (order==1){
        ...
        gw.drawLine(x+size,y,x+0.5*size,y+0.5*size*sqrt(3));
        gw.drawLine(x,y,x+0.5*size,y+0.5*size*sqrt(3));

    }else if(order>1){
        drawSierpinskiTriangle(gw,x,y, 0.5*size, order-1);
        drawSierpinskiTriangle(gw,x+0.5*size,y, 0.5*size, order-1);
        drawSierpinskiTriangle(gw,x+0.25*size,y+0.25*size*sqrt(3), 0.5*size, order-1);
    }else if (order<1){
...
void mandelbrotSet(GWindow& gw, double minX, double incX,
    ...
    for (int r=0;r<pixels.numRows();r++){
        for (int c=0;c<pixels.numCols();c++){
            Complex cplx=Complex(minX+c*incX,minY+r*incY);
            numIterations=mandelbrotSetIterations(cplx,maxIterations);
...
int mandelbrotSetIterations(Complex c, int maxIterations) {
    ...
    return maxIterations-remainingIterations; // Only here to make this compile
}
/**
...
int mandelbrotSetIterations(Complex z, Complex c, int remainingIterations) {
    ...
    if(remainingIterations>0){
        ...
        if(f.abs()>4){
            ...
            remainingIterations= mandelbrotSetIterations(f, c, remainingIterations-1);
        }
    }

    return remainingIterations; // Only here to make this compile
}
